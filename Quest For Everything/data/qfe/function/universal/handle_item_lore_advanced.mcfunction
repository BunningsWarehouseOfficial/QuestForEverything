# Check if the item exists and store success (1 if found, 0 if not found)
$execute store success score @s ItemFound run data get entity @s Inventory[{id:'$(item)',components:{$(components)}}]

# Only get the slot if the item was actually found
$execute if score @s ItemFound matches 1 store result score @s ItemSlot run data get entity @s Inventory[{id:'$(item)',components:{$(components)}}].Slot

# When not takeItems and item was found, add player name and playtime lore to the item in the determined slot
execute if score takeItems GameSetup matches 0 if score @s ItemFound matches 1 run function qfe:universal/add_item_lore_by_slot
